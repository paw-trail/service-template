...
d.node("cf",750,120,300,80,"plat","config-server  :8888","라우트 18 · 공개키 · permit-all 9")
d.node("eu",1300,120,300,80,"plat","eureka-server  :8761","이름 → 주소")
d.node("dom",1300,380,300,110,"dom","도메인 서비스 13개","auth · user · pet · place …|헤더만 믿고 토큰은 안 봄")
...
d.node("dom",180,500,260,110,"dom","도메인 서비스 13개","기동할 때 등록|30초마다 하트비트")
...
# ── common
d=D(1500,760)
d.me("cm",750,380,340,110,"lib","common  0.0.14","자동 설정 7개 · Flyway V1 · V2|실행되지 않는 jar")
d.node("gp",750,120,320,80,"ext","GitHub Packages","publish 로 올림 · 덮어쓰기 불가")
d.node("dom",1300,240,320,110,"dom","도메인 서비스 13개","build.gradle 한 줄로 받음|응답 형식 · 예외 · 인증 · 감사 · Outbox")
d.node("nodb",1300,520,320,90,"domn","DB 없는 서비스 3개","verdict · weather · extract|JPA · Kafka · Outbox 는 안 켜짐")
...
d.edge("gp","r","dom","l",L,"commonVersion=0.0.14",via=[(1000,120),(1000,240)],lx=1000,ly=180)
d.edge("cm","r","dom","l",L,"자동 설정 7개 전부",via=[(1000,380),(1000,240)],lx=1010,ly=320,anchor="start")
d.edge("cm","r","nodb","l",L,"Web · Security · Async · RestClient",via=[(1000,380),(1000,520)],lx=1010,ly=460,anchor="start")
...
# ── config (저장소)
d=D(1500,760)
d.me("cf",750,380,340,110,"lib","paw-trail/config","yml 22개 · 4계층|코드 없음 · 공개 저장소")
d.node("cs",180,380,280,90,"plat","config-server","요청마다 여기를 읽음")
d.node("all",180,140,280,100,"dom","서비스 16개","각자 application.yml 은 3줄|나머지는 여기서")
d.node("env",750,120,340,80,"ext","환경변수","${DB_HOST} · ${SERVICE_DB_PASSWORD} · ${AUTH_…}",dash=True)
d.node("gw",1300,200,300,90,"edge","gateway-server.yml","라우트 18 · 공개키 · permit-all 9|290줄")
...
d.node("cm",180,150,260,90,"lib","common","gradle 한 줄 · 자동 설정 7개")
...
# ── weather-service
d=D(1860,960)
d.me("wt",820,450,400,110,"dom","weather-service  :8090",
     "좌표 · 지역의 지금 날씨와 오늘 비|API 1개  (공개 1) · DB 없음 · 이벤트 없음")
d.node("gw",180,300,250,90,"edge","gateway-server","토큰 검증|X-User-Id · X-User-Role 주입")
d.node("fr",180,580,250,110,"ext","웹 화면 — 날씨 띠","메인 · 검색 · 장소 상세|일정 · 최근 본 장소|내 위치는 보내지 않음")
d.node("cf",820,120,300,80,"plat","config-server","포트 · 기상청 주소 · 키 자리 · 차단기 값")
d.node("eu",1560,120,300,80,"plat","eureka-server","등록 · lb:// 해석")
d.node("km",1560,380,300,110,"ext","기상청 단기예보","공공데이터포털 getVilageFcst|하루 8번 발표 · 10분 뒤부터|틀린 키는 403 + 게이트웨이 봉투",dash=True)
d.node("rd",1560,640,300,110,"data","Redis  :6379","weather:forecast:{nx}:{ny}:{발표}|격자 · 발표마다 7시간|직전 발표 폴백도 여기서")
d.node("pm",820,780,320,80,"obs","Prometheus","resilience4j_circuitbreaker_state|name=kma",dash=True)
d.edge("fr","t","gw","b",B,"고른 지역만 (sidoCode · sigunguName)",lx=196,ly=430,anchor="start")
d.edge("gw","r","wt","l",B,"GET /api/v1/weather",
       via=[(470,300),(470,450)],b_pt=(620,450),lx=318,ly=288,anchor="start")
d.edge("cf","b","wt","t",V,"기동 시 설정")
d.edge("wt","r","eu","l",V,"등록",via=[(1120,430),(1120,120)],a_pt=(1020,430),lx=1132,ly=205,anchor="start")
d.edge("wt","r","km","l",O,"회로 차단기 kma · 재시도는 05 만",dash=True,
       a_pt=(1020,450),via=[(1240,450),(1240,380)],b_pt=(1410,380),lx=1252,ly=368,anchor="start")
d.edge("wt","r","rd","l",O,"격자 + 발표로 읽고 씀",both=True,
       a_pt=(1020,480),via=[(1280,480),(1280,640)],b_pt=(1410,640),lx=1292,ly=560,anchor="start")
d.edge("pm","t","wt","b",P,"/actuator/prometheus",dash=True,lx=832,ly=640,anchor="start")
d.note(40,880,"캐시 → 기상청 → 자료 없음(03)이면 직전 발표 · 못 부르면 캐시의 직전 발표(stale) · 둘 다 없으면 503 WEATHER_UNAVAILABLE — 화면은 날씨 자리를 숨김")
d.note(40,902,"좌표는 활용가이드 변환식으로, 지역은 레포에 담은 기상청 격자 표(2026.7 판 · 시도 16 · 시군구 256)로 격자를 찾음 · 광주 · 전남은 12 · 못 찾으면 시도 대표")
d.note(40,924,"실패가 쌓이면 차단기가 열려 30초 동안 기상청을 부르지 않음 · 다른 서비스는 부르지 않음 · 키는 ingest 와 같은 공공데이터포털 계정 키")
d.save("weather-service","weather-service 를 중심으로 · 직접 연결된 것만")

# ── review-service
d=D(1860,960)
d.me("rv",820,450,380,110,"dom","review-service  :8094",
     "후기 · 별점 3문항 · 사진 · 반려동물 여러 마리|API 13개  (공개 9 · 관리자 1 · internal 3)")
d.node("gw",180,450,240,90,"edge","gateway-server","토큰 검증|X-User-Id · X-User-Role 주입")
d.node("cf",820,120,300,80,"plat","config-server","포트 · DB · 추천 태그 · S3 값")
d.node("eu",1560,120,300,80,"plat","eureka-server","등록 · lb:// 해석")
d.node("pg",1560,300,300,110,"data","PostgreSQL  review_db",
       "place_review · review_pet · review_like|좋아요 수는 트리거가 유지함|Redis 안 씀")
d.node("pt",1560,540,300,90,"domn","pet-service","쓸 때 스냅샷 (견종 · 체중 · 크기)|GET /internal/pets?ids=  못 받으면 작성 실패")
d.node("pl",1560,690,300,90,"domn","place-service","내 후기의 장소 이름 ↔ 상세의 후기 수|양방향")
d.node("us",1560,840,300,90,"domn","user-service","목록의 작성자 이름 ↔ 하루 요약|양방향")
d.node("s3",220,690,260,80,"data","AWS S3","reviews/{계정}/…|올리기 · 보기 주소 서명")
d.node("sc",730,840,280,80,"domn","search-service","카드의 후기 수 · 평균 별점")
d.node("kf",820,690,240,80,"data","Kafka","account.withdrawn 수신|발행은 없음")
d.node("au",400,840,200,80,"domn","auth-service","탈퇴를 알림")
d.edge("gw","r","rv","l",B,"공개 9 · 관리자 1",lx=480,ly=434)
d.edge("cf","b","rv","t",V,"기동 시 설정")
d.edge("rv","r","eu","l",V,"등록",via=[(1120,450),(1120,120)],lx=1200,ly=205)
d.edge("rv","r","pg","l",O,"JPA · Flyway V20 · V21",via=[(1120,450),(1120,300)],lx=1200,ly=378)
d.edge("rv","r","pt","l",G,"쓸 때 한 번",a_pt=(1010,478),via=[(1160,478),(1160,540)],lx=1085,ly=532)
d.edge("rv","r","pl","l",G,"이름 ↔ 후기 수",both=True,a_pt=(1010,495),via=[(1250,495),(1250,690)],lx=1262,ly=648,anchor="start")
d.edge("rv","r","us","l",G,"이름 ↔ 기간 목록",both=True,a_pt=(1010,502),via=[(1330,502),(1330,840)],lx=1342,ly=800,anchor="start")
d.edge("rv","l","s3","t",O,"사진 주소 서명",a_pt=(700,505),via=[(700,600),(220,600)],b_pt=(220,650),lx=688,ly=580,anchor="end")
d.edge("sc","t","rv","b",G,"후기 수 · 평균",a_pt=(730,800),via=[(730,750),(880,750)],b_pt=(880,505),lx=742,ly=772,anchor="start")
d.edge("kf","t","rv","b",O,"탈퇴하면 그 사람 후기를 지움",a_pt=(820,650),b_pt=(820,505),lx=832,ly=588,anchor="start")
d.edge("au","t","kf","l",X,"account.withdrawn",a_pt=(400,800),via=[(400,720),(700,720)],lx=388,ly=742,anchor="end")
d.note(40,900,"후기를 쓸 때만 pet 을 부르고 그때 견종 · 체중 · 크기를 복사해 둠 — 못 받으면 작성을 실패시킴 (그때만 찍을 수 있는 값이라 나중에 채울 길이 없음)")
d.note(40,922,"place · user 는 양방향 — review 가 이름을 받아 오고, 그쪽은 후기 수(장소 상세)와 기간 목록(하루 요약)을 받아 감 · 이름을 못 받으면 그 칸만 비운 채 목록을 냄")
d.note(40,944,"이벤트는 받기만 함 (Outbox 표는 비어 있음) · 사용자 삭제는 행을 남기고 탈퇴는 실제로 지움 · 좋아요 수는 review_like 의 트리거가 ±1 함")
d.save("review-service","review-service 를 중심으로 · 직접 연결된 것만")
